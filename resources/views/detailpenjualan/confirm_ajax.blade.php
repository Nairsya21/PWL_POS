...
@else
    <form action="{{ url('/detailpenjualan/' . $detail->detail_id . '/delete_ajax') }}" method="POST" id="form-delete-detail-penjualan">
        @csrf
        @method('DELETE')
        <div id="modal-master" class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="alert alert-warning">
                        <h5><i class="icon fas fa-ban"></i> Konfirmasi !!!</h5> Apakah Anda ingin menghapus data detail penjualan seperti di bawah ini?
                    </div>
                    <table class="table table-sm table-bordered table-striped">
                        <tr>
                            <th class="text-right col-3">Kode Transaksi :</th>
                            <td class="col-9">{{ $detail->penjualan->penjualan_kode }}</td>
                        </tr>
                        <tr>
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.status) {
                                ...
                                dataDetailPenjualan.ajax.reload(); // Make sure this matches your DataTable instance for penjualan
                            } else {
                                $('.error-text').text('');
